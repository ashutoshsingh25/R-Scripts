...
MCAT_Training_PATH <- paste0("E:\\Product Classification_new\\NPS\\Child\\")
PMCAT_Testing_PATH <- paste0("E:\\Product Classification_new\\NPS\\PMCAT\\")
Kfold_PATH <- paste0("E:\\Product Classification_new\\NPS\\Kfold/")
...
df1 <- df[,c("PRIME_MCAT_ID","PMCAT2","PRIME_MCAT_NAME","OTHER_MCATS","PC_ITEM_ID","PC_ITEM_NAME","PC_ITEM_DESC_SMALL","FK_IM_SPEC_MASTER_DESC","FK_IM_SPEC_OPTIONS_DESC","Label","Label1")]
#df1 <- df[,c(1:5,8,9)]
#fix(df)
#df2 <- df1[1:2000,]
...
table(df2$PMCAT2)
#df2$OTHER_MCATS <- gsub(",*")

gsub(",\\w+","","excavator,backhoe")

#sapply(strsplit("Motor Grader, Backhoe Loader",","),"[",1)
df2$OTHER_MCATS <- sapply(strsplit(df2$OTHER_MCATS, ","),"[",1)
...
data_unique$PRIME_MCAT_NAME1 <- paste0("p_",data_unique$PRIME_MCAT_NAME1)
data_unique$PRIME_MCAT_NAME2 <- gsub(" ","_",data_unique$OTHER_MCATS)
data_unique$PRIME_MCAT_NAME2 <- paste0("s_",data_unique$PRIME_MCAT_NAME2)
data_unique[data_unique$PRIME_MCAT_NAME2=="s_NA","PRIME_MCAT_NAME2"] <- ""
sum(is.na(data_unique$PRIME_MCAT_NAME2))
for (m in 1:nrow(data_unique)) {
  ifelse(data_unique$PRIME_MCAT_NAME2[m]=="",data_unique$PRIME_MCAT_NAME2[m] <- "",data_unique$PRIME_MCAT_NAME2[m] <- paste0("__label__",data_unique$PRIME_MCAT_NAME2[m]))
}


#data_unique$PRIME_MCAT_NAME1 <- gsub(" ","_",data_unique$PMCAT2)
data_unique$Fasttext_label <- paste0("__label__",data_unique$PRIME_MCAT_NAME1," ",data_unique$PRIME_MCAT_NAME2," ",data_unique$concat_name)

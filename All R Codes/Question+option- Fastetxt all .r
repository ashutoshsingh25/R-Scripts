...
df<-readxl::read_excel("C:/Users/Imart/Desktop/QuesndOpt/AC/AC.xlsx",sheet = 1 )# Path of excel containing, MCAT ID and their corresponding labels
...
df$FK_IM_SPEC_OPTIONS_DESC <- gsub("yes","",df$FK_IM_SPEC_OPTIONS_DESC)
df$FK_IM_SPEC_OPTIONS_DESC <- gsub("no","",df$FK_IM_SPEC_OPTIONS_DESC)
df$IM_SPEC_MASTER_DESC <- gsub("[[:digit:]]", "", df$FK_IM_SPEC_MASTER_DESC)
df$IM_SPEC_MASTER_DESC <- gsub("<.*?>", " ", df$FK_IM_SPEC_MASTER_DESC)
df$IM_SPEC_MASTER_DESC <- tolower(df$FK_IM_SPEC_MASTER_DESC)
df$concat <- gsub(" ","_",df$IM_SPEC_MASTER_DESC)
df$IM_SPEC_MASTER_DESC <- gsub("*\\(.*?\\)","",df$IM_SPEC_MASTER_DESC)
df$label <-  paste0("__label__",df$PC_ITEM_ATTRIBUTE_MCATID,"_",df$concat," ",df$IM_SPEC_MASTER_DESC," ",df$FK_IM_SPEC_OPTIONS_DESC)
...
for(i in 1:length(out)){
  ...
  location<-paste0("C:/Users/Imart/Desktop/QuesndOpt/AcwithoutYN/",location)
...
df$test <- paste0(df$IM_SPEC_MASTER_DESC," ",df$FK_IM_SPEC_OPTIONS_DESC)
...
colnames(df)[colnames(df)=='test'] <- 'value'
df$RN <- NULL
fix(df)
out <- split(df,f = df$PC_ITEM_ATTRIBUTE_MCATID) 
i <- 1
...
for(i in 1:length(out))
{
  ...
  for(j in 1:length(a))
  {
    ...
    mainDir<-"C:/Users/Imart/Desktop/QuesndOpt/AcwithoutYN/testing/"
    subDir<-hold

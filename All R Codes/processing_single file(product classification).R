...
#df <- read.csv("E:/Conduit/PMCAT_wise_child label/final/processed/process2/Cable_raceways.csv",header = F)

Files <- list.files(path = "E:\\Conduit\\Bl-testing/final/",pattern = ".csv$")
Files[1]
Files <- gsub(".csv","",Files)
j <- 1

for (j in 1:6) {
  locations <- paste0("E:\\Conduit\\Bl-testing/final/",Files[j],".csv")
  locations <- paste0("E:/Yarn_new/Cotton_yarn.csv")
  ...
  location2 <- paste0("E:/Yarn_new/Cotton_yarn",j,".csv")
  write.csv(final_out,location2,row.names = F,quote = F)
  
  
  write.xlsx(final_out,"file:///E:/Yarn_new/Cotton_yarn.xlsx",row.names = FALSE,sheetName = Files[j],append = TRUE)
}
{

...
MCAT_Training_PATH <- paste0("c:/Users/imart/Desktop/Excavator_With Search/MCAT/")
PMCAT_Testing_PATH <- paste0("c:/Users/imart/Desktop/Excavator_With Search/PMCAT/")
Kfold_PATH <- paste0("c:/Users/imart/Desktop/Excavator_With Search/Kfold/")
...
colnames(df)[length(colnames(df))] <- "Search Top"
varr <- names(df)
varr
...
df1 <- df[,c("PRIME_MCAT_ID","PMCAT2","PRIME_MCAT_NAME","PC_ITEM_ID","PC_ITEM_NAME","PC_ITEM_DESC_SMALL","FK_IM_SPEC_MASTER_DESC","FK_IM_SPEC_OPTIONS_DESC","Label","Label1","Search Top")]
#df1 <- df[,c(1:5,8,9)]
#fix(df)
#df2 <- df1[1:2000,]
...
sum(is.na(data_unique$`Search Top`)
    
    data_unique[is.na(data_unique$`Search Top`),"Search Top"] <- ""
    
    data_unique$Label1 <- gsub("[0-9]","",data_unique$Label1)
    #data_unique$Label1 <- tolower(data_unique$Label1)
    data_unique$concat_name <- paste(data_unique$PC_ITEM_NAME1,data_unique$Label1,data_unique$New_Desc,tolower(data_unique$`Search Top`))
    ...
    data_unique2 <- data_unique[,c(1:5,9,13,14)]
    
    #write.csv(data_unique2,PMCAT_TRAINING_LOC2,row.names = F,quote = F)
    ...
    out2 <- split(data_unique2,f=data_unique$PMCAT2)

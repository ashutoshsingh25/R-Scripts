...
df3 <- dfC[1:15000,c(10,2)]
class(df3$Sold_Ratio_UBL)
#fix(df3)
#df3[df3$Sold_Ratio_UBL==0,1] <- 1
df3$GLCAT_MCAT_NAME <- as.factor(df3$GLCAT_MCAT_NAME)
class(df3$GLCAT_MCAT_NAME)
c4 <- mdlp(df3)
c4 <- c4$Disc.data
c5 <- cbind(c4,df3)
c5 <- data.frame(c5[,c(3,1)])

#MDL_A <- merge.data.frame(x=dfC,y=c5,by.x = "Sold_Ratio_UBL",by.y = "Sold_Ratio_UBL",all.x = T)
#MDL_A <- merge(x=dfC,y=c5,by.x = "Sold_Ratio_UBL",by.y = "Sold_Ratio_UBL",all.x = T)
#df1<-dfC[,c(8,2)]
df1 <- dfC[,c(2,4,9,10)]
df2 <- df1[,c(1,3,4)]
class(df2$Sold_Ratio_UBL)

descretized_df <- discretizeDF(df2$Sold_Ratio_TBL)
descretized_df2 <- discretizeDF(df2$Sold_Ratio_UBL)
desc_f22 <- as.data.frame(cbind(descretized_df,descretized_df2))
df_res <- df1[,c(3,1)]
df_res <- df_res[!is.na(df_res$GLCAT_MCAT_NAME),]
df_res$GLCAT_MCAT_NAME <- as.factor(df_res$GLCAT_MCAT_NAME)


mdl_UBL <- mdlp(df_res)$Disc.data
mdl_TBL2 <- discretize(dfC$Sold_Ratio_TBL,breaks = 1)
?discretize
mdl_UBL2 <- discretize(df_res$Sold_Ratio_UBL,breaks = 1)
MDL_final <- as.data.frame(cbind(mdl_TBL2,mdl_UBL2))

MDL_final <- cbind(dfC,MDL_final)
MDL_final <- 
...
dfc2 <- readxl::read_excel("C:/Users/Imart/Downloads/o_a.xlsx",sheet = 1)
df3<-dfc2[,c(10,2)]
class(df3$`PMCAT Name`)
